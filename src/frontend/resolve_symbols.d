...
{
    static Node[] resolve(Diagnostics err, ImportResolverContext* context, Node[] body)
    {
        Node[] newBody;
        foreach (Node node; body)
        {
            ...
            if (name != "")
                if (Position* p = name in context.symbols)
                {
                    if (p.filename == node.pos.filename)
                    {
